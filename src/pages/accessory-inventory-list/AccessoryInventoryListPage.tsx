// Path: src/pages/accessory-inventory-list/AccessoryInventoryListPage.tsx

import { useState, useEffect, useMemo, type FC } from "react";
import { AccessoryInventoryHeader } from "./components/AccessoryInventoryHeader";
import { AccessoryInventoryFilters } from "./components/AccessoryInventoryFilters";
import { AccessoryInventoryTable } from "./components/AccessoryInventoryTable";
import Pagination from "./components/Pagination";
import type { AccessoryItem, AccessoryStatus } from "./types";
import { FilterSkeleton } from "./components/skeletons/FilterSkeleton";
import { TableSkeleton } from "./components/skeletons/TableSkeleton";
import { DUMMY_ACCESSORY_DATA } from "./data";
...
const AccessoryInventoryListPage = () => {
  ...
  const handleExecuteIssue = (
    ...
    setAccessoryItems((prevItems) =>
      prevItems.map((item) => {
        if (item.id === itemToUpdate.id) {
          const newQuantity = item.quantity - quantityToIssue;
          let newStatus: AccessoryStatus = "In Stock";
          if (newQuantity <= 0) {
