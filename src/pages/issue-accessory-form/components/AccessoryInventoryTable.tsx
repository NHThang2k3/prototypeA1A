// Path: src/pages/issue-accessory-form/components/AccessoryInventoryTable.tsx

import React, { useState, useEffect, useMemo } from "react";
import { Loader2 } from "lucide-react";
import type { AccessoryInventoryItem, SelectedAccessoryItem } from "../types";
import { getAccessoryInventory } from "../data";
import ColumnSelector from "./ColumnSelector";

interface Props {
  bomId: string; // Used to fetch relevant items
  onSelectionChange: (items: SelectedAccessoryItem[]) => void;
}
...
const AccessoryInventoryTable: React.FC<Props> = ({
  bomId,
  onSelectionChange,
}) => {
  const [inventory, setInventory] = useState<AccessoryInventoryItem[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    setSelected(new Map());
    getAccessoryInventory(bomId).then((data) => {
      setInventory(data);
      setLoading(false);
    });
  }, [bomId]);
  ...
  useEffect(() => {
    const selectedItemsData: SelectedAccessoryItem[] = [];
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mt-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-gray-700">
          2. Select Accessories from Inventory
        </h2>
        <ColumnSelector
                  ...
                  (col) =>
                    visibleColumns.has(col.key) && (
                      <td key={col.key} className="p-3 whitespace-nowrap">
                        {item[col.key as keyof AccessoryInventoryItem]}
                      </td>
                    )
                )}
                <td className="p-2">
...
export default AccessoryInventoryTable;
