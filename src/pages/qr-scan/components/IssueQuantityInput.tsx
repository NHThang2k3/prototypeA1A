...
const IssueQuantityInput: React.FC<IssueQuantityInputProps> = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (numQuantity > maxCanTake) {
      toast.error(`Quantity cannot exceed ${maxCanTake} ${item.uom}.`);
      return;
    }
      ...
      >
        ...
        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-3 rounded-md mb-4 text-sm">
          ...
          <p className="font-bold mt-1">
            {" "}
            == Max can take: {maxCanTake.toLocaleString()} {item.uom}
          </p>
        </div>
        ...
        <div className="mb-4">
          ...
          >
            Actual quantity to issue ({item.uom})
          </label>
          <input
            type="number"
            id="quantity"
            ...
            placeholder={`Enter number of ${item.uom.toLowerCase()}...`}
            autoFocus
            step="any"
          />
        </div>
