// Path: src/pages/qr-scan/flows/ScanQRWarehouseLocation.tsx
...
// ... (All type definitions from the previous version remain the same)
export type ScanAction = "PUT_AWAY" | "TRANSFER" | "VIEW_DETAIL";
export type PickingListItem = {
  sku: string;
  name: string;
  uom: "Roll" | "Box" | "Meters" | "Piece";
...
// --- MOCK DATA & API (No changes here) ---
// ... (All mock data and API functions from the previous version remain the same)
...
const submitPutAwayAction = (
  ...
  new Promise((resolve) => {
    setTimeout(() => {
      console.log(
        `Putting away ${items.length} fabric rolls to location ${location.locationCode}`
      );
      resolve({
        success: true,
        message: `Success! Moved ${items.length} fabric rolls to location ${location.locationCode}.`,
      });
    }, 1000);
  });

// --- UI SUB-COMPONENTS (With modifications) ---

// Scanner Component (No changes)
...
// ActionFeedback Component (No changes)
...
// **MODIFIED** Component: InboundPutAway
interface InboundPutAwayProps {
  location: ScannedLocation;
  scannedItemCount: number; // Changed from scannedItems array to just a count
  onScanItem: (qrCode: string) => void;
  onComplete: () => void; // Changed from onSubmit
  onCancel: () => void;
}
const InboundPutAway: React.FC<InboundPutAwayProps> = ({
  ...
  <div className="w-full max-w-lg mx-auto p-4 animate-fade-in">
    <div className="bg-white rounded-lg shadow-lg p-6 border border-gray-200">
      ...
      {/* The list of items is now hidden from this screen */}
      <div className="text-center my-4 p-4 bg-gray-50 rounded-lg">
        <h3 className="font-semibold text-lg text-gray-800">
          {scannedItemCount}
        </h3>
        <p className="text-sm text-gray-600">Fabric Roll(s) Scanned</p>
      </div>

      <div className="mt-4">
        <Scanner
          onScan={onScanItem}
          scanPrompt="Scan QR codes on fabric rolls"
        />
      </div>
    </div>
...
// **NEW** Component: ConfirmationScreen
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 border border-gray-200">
      <div className="text-center mb-4 border-b pb-4">
        ...
        <p className="text-sm text-gray-500">
          Putting away {items.length} roll(s) to location:
        </p>
        <p className="font-mono text-blue-600 bg-blue-100 px-3 py-1 rounded-full inline-block mt-1">
          {location.locationCode}
        </p>
...
const ScanQRWarehouseLocation: React.FC = () => {
  ...
  const handleScan = async (qrCode: string) => {
    // Scanning logic only applies when in the first two states
    if (
      ...
      if (state.name === "AWAITING_LOCATION_SCAN") {
        ...
        if (data.type === "item") {
          ...
          if ((data as ScannedItem).currentLocation) {
            toast.error(
              `Warning: Fabric already has a location (${
                (data as ScannedItem).currentLocation
              }).`
            );
          }
          // Just show success, the UI will only update the count
          setState({ ...state, items: [...state.items, data as ScannedItem] });
          toast.success(`Scanned: ${data.sku}`);
        } else {
          toast.error("Please scan a FABRIC ROLL QR code.");
        }
      }
    } catch (error) {
  ...
  const handleProceedToConfirmation = () => {
    ...
    // Transition to the new confirmation state
    setState({
      name: "AWAITING_CONFIRMATION",
  ...
  const handleBackToScanning = () => {
    if (state.name !== "AWAITING_CONFIRMATION") return;
    // Go back to the scanning state, keeping the current items
    setState({
      name: "SCANNING_ITEMS",
      location: state.location,
      items: state.items,
    ...
    switch (state.name) {
      case "AWAITING_LOCATION_SCAN":
        return (
          <Scanner
            onScan={handleScan}
            scanPrompt="Scan a Warehouse Location QR to start Put Away"
          />
        );
      case "SCANNING_ITEMS":
        return (
...
export default ScanQRWarehouseLocation;
