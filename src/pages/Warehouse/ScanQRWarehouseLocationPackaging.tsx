// Path: src/pages/qr-scan/flows/ScanQRWarehouseLocationPackaging.tsx
...
const submitPutAwayAction = (
  ...
  new Promise((resolve) => {
    setTimeout(() => {
      console.log(
        `Putting away ${items.length} packaging items to location ${location.locationCode}`
      );
      resolve({
        success: true,
        message: `Success! Moved ${items.length} packaging items to location ${location.locationCode}.`,
      });
    }, 1000);
  });
  ...
  <div className="w-full max-w-md mx-auto p-4 flex flex-col items-center">
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-3">
        ...
        >
          Scan Location P-02-C
        </Button>
        <Button onClick={() => onScan("ITEM_QR_PKG_002")} className="w-full">
          Scan Polybag (No Location)
        </Button>
        <Button onClick={() => onScan("ITEM_QR_PKG_003")} className="w-full">
          Scan Packing Tape (No Location)
        </Button>
        <Button
      ...
      <CardContent className="pt-4">
        <div className="text-center my-4 p-4 bg-muted rounded-lg">
          ...
          <p className="text-sm text-muted-foreground">
            Packaging Item(s) Scanned
          </p>
        </div>
        <div className="mt-4">
          <Scanner
            onScan={onScanItem}
            scanPrompt="Scan QR codes on packaging boxes/items"
          />
        </div>
      </CardContent>
    </Card>
...
const ScanQRWarehouseLocationPackaging: React.FC = () => {
      ...
      if (state.name === "AWAITING_LOCATION_SCAN") {
        ...
        if (data.type === "item") {
          if (state.items.some((item) => item.qrCode === data.qrCode)) {
            toast.error("This packaging item has already been scanned.");
            return;
          }
          if ((data as ScannedItem).currentLocation) {
            toast.error(
              `Warning: Packaging item already has a location (${
          ...
          toast.error("Please scan a PACKAGING item QR code.");
        }
      }
    } catch (error) {
    ...
    switch (state.name) {
      case "AWAITING_LOCATION_SCAN":
        return (
          <Scanner
            onScan={handleScan}
            scanPrompt="Scan a Warehouse Location QR to start Put Away for Packaging"
          />
        );
      case "SCANNING_ITEMS":
        return (
...
export default ScanQRWarehouseLocationPackaging;
