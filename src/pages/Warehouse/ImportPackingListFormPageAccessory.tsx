// src/pages/import-packing-list/ImportPackingListFormPageAccessory.tsx
...
interface PackingListItem {
  ...
  boxNo: string;
  lotNo: string;
  quantity: number; // Replaces 'yards'
  unit: string; // e.g., PCS, SET
  netWeight: number;
  grossWeight: number;
  size: string; // Accessory specific
  location: string;
  qrCode: string;
  description: string;
}
...
const ImportPackingListFormPageAccessory: React.FC = () => {
  ...
  const handleSubmit = () => {
    ...
    console.log("Submitting the following accessory items:", items);
    setTimeout(() => {
      console.log("Submission successful!");
      setIsSubmitting(false);
      setItems([]);
      alert("Inbound accessory shipment created successfully!");
    }, 2000);
  };
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              Inbound Accessories from Packing List
            </h1>
            <p className="mt-1 text-sm text-gray-500">
              Upload an Excel file to create a batch inbound accessory shipment.
            </p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">
            1. Upload Accessory Packing List File
          </h2>
          <div>
...
export default ImportPackingListFormPageAccessory;
